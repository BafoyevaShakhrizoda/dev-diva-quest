...
import { demandLabel } from "@/i18n/careerLabels";
...
const CareerDetail = ({ career, onClose }: CareerDetailProps) => {
  ...
  const fetchJobs = useCallback(async () => {
    setLoadingJobs(true);
    try {
      const response = await apiClient.getAllJobs({ role: career.id });
      const list = Array.isArray(response) ? (response as Job[]) : [];
      setJobs(list.slice(0, 5));
    } catch {
      toast.error(t("jobs.applyFail"));
    } finally {
    ...
    >
      ...
      <div className="relative w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[85vh] overflow-y-auto gradient-card rounded-t-3xl sm:rounded-2xl shadow-hover border border-border animate-scale-in">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-body text-muted-foreground">
                {t("careerDetail.demandLine", { demand: demandLabel(locale, career.demand) })}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex border-b border-border">
          ...
          >
            {t("careerDetail.jobsTab", { count: jobs.length })}
          </button>
        </div>
        ...
        <div className="px-6 py-6">
          {activeTab === 'overview' ? (
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-body font-semibold text-sm uppercase tracking-widest text-muted-foreground mb-3">
                  {t("careerDetail.toolsFrameworks")}
                </h3>
                <div className="flex flex-wrap gap-2">
              ...
              {/* Salary */}
              <div>
                <h3 className="font-display text-lg font-bold text-foreground mb-3">{t("careerDetail.salaryRange")}</h3>
            ...
            <div className="space-y-4">
              {loadingJobs ? (
                <div className="text-center py-8">
                  ...
                  <p className="mt-3 text-sm text-muted-foreground">{t("careerDetail.loadingJobs")}</p>
                </div>
              ) : jobs.length === 0 ? (
                <div className="text-center py-8">
                  <Briefcase className="w-12 h-12 mx-auto mb-3 text-muted-foreground" />
                  <h3 className="font-display text-lg font-bold text-foreground mb-2">{t("careerDetail.noJobsTitle")}</h3>
                  <p className="text-sm text-muted-foreground">{t("careerDetail.noJobsHint")}</p>
                </div>
              ) : (
                jobs.map((job) => (
                  <div
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {job.remote && (
                        <span className="text-xs rounded-full bg-green-500/15 px-2 py-1 font-medium text-green-700 dark:text-green-400">
                          {t("jobs.remote")}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-3 text-xs text-muted-foreground">
                      ...
                      {job.salary_min != null && job.salary_max != null && (
                        <span className="flex items-center gap-1 tabular-nums">
                          ...
                          {formatSalaryUzRange(job.salary_min, job.salary_max, locale)}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-muted-foreground">{t("careerDetail.postedRecently")}</div>
                      <button
                        type="button"
